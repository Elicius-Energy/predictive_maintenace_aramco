...
const Topbar: FC = () => {
  ...
  return (
    <header className="min-h-[4rem] py-2 glass-topbar flex flex-wrap items-center justify-between px-4 lg:px-6 z-40 gap-y-3">
      {/* breadcrumbs / machine selector */}
      <div className="flex items-center gap-3 lg:gap-6 flex-wrap">
        <div className="flex items-center gap-2 px-3 py-1.5 glass-badge rounded-lg shrink-0">
        ...
        <div className="h-6 w-px bg-white/40 hidden sm:block" />
        ...
        <div className="relative group min-w-[140px] md:min-w-[180px] shrink-0">
          ...
          >
            {machines.map(m => (
              <option key={m.machine_id} value={m.machine_id} className="bg-white text-text-primary">
                {m.name}
              </option>
            ))}
          </select>
        ...
        <div className="flex items-center gap-2 lg:gap-3 glass-badge p-1.5 rounded-lg shrink-0">
          <div className="flex items-center gap-1.5">
            ...
            <input 
              ...
              className="glass-input text-xs font-mono font-bold text-text-primary px-2 py-1 rounded-lg disabled:opacity-50"
          ...
          <button 
            ...
            className="flex items-center gap-1.5 px-2 py-1 rounded-lg hover:bg-white/40 transition-colors border border-transparent hover:border-white/50 text-text-secondary hover:text-primary disabled:opacity-50"
      ...
      <div className="flex items-center gap-4 lg:gap-8 flex-wrap shrink-0">
        ...
        {/* Health Indicator */}
        <div className="flex items-center gap-3 px-4 py-2 glass-badge rounded-full">
          ...
          <div className={cn(
            ...
            healthStatus === 'healthy' ? "bg-accent-green shadow-[0_0_8px_#059669]" : 
            healthStatus === 'warning' ? "bg-accent-amber shadow-[0_0_8px_#d97706]" : "bg-accent-red shadow-[0_0_8px_#dc2626]"
          )} />
        </div>

        {/* Notifications */}
        <div className="relative cursor-pointer group">
          <div className="hover:bg-white/40 p-2 rounded-full transition-colors relative">
            <Bell size={20} className={cn("text-text-muted", activeAlerts.length > 0 && "animate-pulse text-accent-amber")} />
            {activeAlerts.length > 0 && (
              <span className="absolute top-0 right-0 w-4 h-4 bg-accent-red text-white text-[10px] flex items-center justify-center rounded-full font-bold shadow-lg shadow-red-500/30">
                {activeAlerts.length}
              </span>
            )}
          </div>
          ...
          <div className="absolute right-0 mt-2 w-80 industrial-card opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 overflow-hidden">
            <div className="p-3 border-b border-white/30 bg-white/30">
              <h3 className="text-sm font-bold text-text-primary">Recent Alerts</h3>
            </div>
            <div className="max-h-64 overflow-y-auto">
              {activeAlerts.length > 0 ? (
                activeAlerts.slice(0, 5).map((alert, idx) => (
                  <div key={idx} className="p-3 border-b border-white/20 hover:bg-white/20 transition-colors">
        ...
        <div className="flex flex-col items-end border-l border-white/30 pl-6 pr-2">
        ...
        {/* LEDL Logo */}
        <div className="border-l border-white/30 pl-6 hidden sm:flex items-center">
