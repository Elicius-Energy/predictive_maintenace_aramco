...
const MotorDetailsForm: FC = () => {
  const navigate = useNavigate();
  const { motorDetails, setMotorDetails } = useMotorDetails();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setMotorDetails(form);
    navigate('/machines');
  };

  const isValid = form.motorName.trim() !== '' && form.ratedPower > 0 && form.ratedEfficiency > 0;

  return (
    <div className="min-h-screen bg-surface-muted overflow-y-auto font-sans">
      {/* Hero header */}
      <div className="bg-gradient-to-br from-primary-dark via-primary to-cyan-400 text-white">
        <div className="max-w-5xl mx-auto px-8 py-12">
          ...
          <div className="flex items-center gap-2 mt-6">
            ...
            <span className="text-cyan-200 text-xs font-medium">Step 1 of 2 — Motor Setup</span>
          </div>
        </div>
      </div>
